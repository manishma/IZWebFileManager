...
declare function WebFileManager_DoCallback(target, argument, clientCallback, context, clientErrorCallback):any;
declare var IZWebFileManager_MouseHover;
declare var IZWebFileManager_MouseOut;
declare var IZWebFileManager_ShowElement;
declare var IZWebFileManager_HideElement;
...
FileManagerController.prototype.OnRename = function(sender, arg) {
    ...
    this.TextBox.onclick = function(e:any) {
        e = e || event;
        e.cancelBubble = true;
    }
    
    this.TextBox.onkeydown = function(e:any) {
        e = e || event;
        if(e.keyCode == 27) {
...
FileManagerController.prototype.OnSelectedItemsCopyTo = function(sender, arg) {
    ...
    this.PromptDirectory(directory, (arg) => this._SelectedItemsCopyTo(sender, arg));
}
...
FileManagerController.prototype.OnSelectedItemsMoveTo = function(sender, arg) {
    ...
    if(sender.SelectedItems.length == 0)
        return;
    var directory = decodeURIComponent(sender.GetDirectory());
    this.PromptDirectory(directory, (arg) => this._SelectedItemsMoveTo(sender, arg));
}
...
FileManagerController.prototype.OnSearch = function (sender, arg) {
    if (sender.InProcess)
        return;
    var search = <HTMLInputElement>WebForm_GetElementById(sender.ClientID + '_SearchTerm');
